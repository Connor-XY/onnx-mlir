...
func.func @test_onnx_to_matmul2d_dyn(%arg0 : tensor<?x?xf32>, %arg1 : tensor<?x?xf32>) -> tensor<*xf32> {
  ...
  // CHECK:     [[VAR_0_:%.+]] = "mhlo.dot"([[VAR_1_:%.+]], [[VAR_2_:%.+]]) : (tensor<?x?xf32>, tensor<?x?xf32>) -> tensor<?x?xf32>
...
func.func @test_onnx_to_matmul3d_dyn(%arg0 : tensor<?x?x?xf32>, %arg1 : tensor<?x?x?xf32>) -> tensor<*xf32> {
  ...
  // CHECK:   [[VAR_0_:%.+]] = "mhlo.dot_general"([[VAR_1_:%.+]], [[VAR_2_:%.+]]) {dot_dimension_numbers = #mhlo.dot<lhs_batching_dimensions = [0], rhs_batching_dimensions = [0], lhs_contracting_dimensions = [2], rhs_contracting_dimensions = [1]>} : (tensor<?x?x?xf32>, tensor<?x?x?xf32>) -> tensor<?x?x?xf32>
